...
def expand_key_128(initial_key):
    ...
    expanded_key[:len(initial_key)] = initial_key[:len(initial_key)]
    ...
    while len(expanded_key) < 176:
        ...
        temp = expanded_key[(len(expanded_key)-4):len(expanded_key)]
        ...
        for itr in range(4):
            expanded_key.append(expanded_key[len(expanded_key)-16] ^ temp[itr])
...
def expand_key_192(initial_key):
    ...
    while len(expanded_key) < 208:
        ...
        for itr in range(4):
            expanded_key.append(expanded_key[len(expanded_key)-24] ^ temp[itr])
...
def expand_key_256(initial_key):
    ...
    while len(expanded_key) < 240:
        ...
        for itr2 in range(4):
            temp[itr2] = sbox[temp[itr2]]
                
        for itr in range(4):
            expanded_key.append(((expanded_key[len(expanded_key)-32]) ^ (temp[itr])))
