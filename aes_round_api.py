...
def add_round_key(input_state, round_key):
    # make a bytearray with size of input_state len 16
...
def sub_bytes(input_state, mode):
    ...
    for i in range(len(input_state)):
        output_state.append(sbox[input_state[i]])
...
def shift_rows(input_state, mode):
    ...
    for i in range(len(input_state)):
        output_state[i*4:i*4+4] = rotate(input_state[i*4:i*4+4], shift * i)
...
def aes_round_dec(input_state, round_key, last_round=False):
    ...
    output_state = shift_rows(input_state, 'D')
    
    # perform inverse substitute byte
    output_state = sub_bytes(output_state, 'D')
